...
export default function CombatScreen({ onExit }: { onExit: () => void }) {
  ...
  return (
    <div className="w-full max-w-6xl h-[800px] flex flex-col bg-slate-950 rounded-xl border-4 border-gold-600 overflow-hidden shadow-2xl relative">
      {/* Background */}
      ...
      <div className="flex-grow relative z-10 flex justify-between items-end px-12 py-12 pb-24">
        
        {/* Player Side */}
        <div className="flex flex-col items-center gap-6 w-1/3 relative">
          {/* Stats Card */}
          <div className="w-full bg-slate-900/90 border border-gold-600/50 p-4 rounded-lg backdrop-blur-md shadow-lg absolute -top-12 left-0 z-20">
            <div className="flex justify-between items-end mb-2">
              <h3 className="text-xl font-serif text-gold-400">{player.name}</h3>
              <span className="text-sm text-slate-400">Lvl {player.level}</span>
            </div>
            {/* HP Bar */}
        ...
        <div className="flex flex-col items-center gap-4 mb-20">
          <div className="text-6xl font-serif text-transparent bg-clip-text bg-gradient-to-b from-gold-300 to-amber-700 font-bold drop-shadow-lg italic">VS</div>
          <motion.div 
            key={currentTurn}
            initial={{ y: 10, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            className={`px-6 py-2 rounded-full border-2 ${currentTurn === "Player" ? "bg-green-950/50 border-green-500 text-green-400 shadow-[0_0_15px_rgba(34,197,94,0.3)]" : "bg-red-950/50 border-red-500 text-red-400 shadow-[0_0_15px_rgba(239,68,68,0.3)]"} text-sm font-bold uppercase tracking-[0.2em] backdrop-blur-md`}
          >
            {currentTurn}'s Turn
          </motion.div>
        </div>
        ...
        <div className="flex flex-col items-center gap-6 w-1/3 relative">
          {/* Stats Card */}
          <div className="w-full bg-slate-900/90 border border-red-900/50 p-4 rounded-lg backdrop-blur-md shadow-lg absolute -top-12 right-0 z-20">
            <div className="flex justify-between items-end mb-2">
              <h3 className="text-xl font-serif text-red-400">{enemy.name}</h3>
              <span className="text-sm text-slate-400">Elite Construct</span>
            </div>
            {/* HP Bar */}
            <div className="w-full h-4 bg-slate-800 rounded-full overflow-hidden border border-slate-700 relative">
              <div className="absolute inset-0 bg-slate-900/50 z-10" />
      ...
      <div className="h-1/3 bg-slate-950 border-t-4 border-gold-600 relative z-20 flex shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        
        {/* Actions */}
        <div className="w-2/3 p-8 grid grid-cols-2 gap-6 bg-[url('/noise.png')]">
          {ACTIONS.map((action) => (
            <button
              key={action.name}
            ...
            >
              ...
              `}>
                {action.type === 'Physical' ? <Sword size={24} /> : <Zap size={24} />}
              </div>
              <div className="text-left z-10">
                <div className={`font-serif text-lg font-bold transition-colors ${currentTurn === "Player" ? "text-slate-200 group-hover:text-gold-300" : "text-slate-500"}`}>
                  {action.name}
                </div>
        ...
        {/* Combat Log */}
        <div className="w-1/3 bg-black/60 border-l-2 border-gold-600/30 flex flex-col">
          ...
          <div className="flex-grow p-4 overflow-y-auto font-mono text-sm space-y-3 custom-scrollbar">
        ...
        {combatStatus !== 'Active' && (
          ...
          >
            <motion.div 
              ...
              className={`
                text-center space-y-8 p-12 rounded-2xl border-2 shadow-2xl max-w-lg w-full
                ${combatStatus === 'Victory' ? 'border-gold-500/30 bg-slate-900/90' : 'border-red-900/30 bg-slate-950/90'}
              `}
            >
              <h2 className={`text-7xl font-serif font-bold ${combatStatus === 'Victory' ? 'text-transparent bg-clip-text bg-gradient-to-b from-gold-300 to-amber-600' : 'text-red-600'} drop-shadow-[0_0_30px_rgba(0,0,0,0.5)]`}>
              ...
              {combatStatus === 'Victory' ? (
                <button 
                  ...
                  className="
                    ...
                    text-gold-100 font-serif text-xl tracking-widest uppercase
                ...
                <div className="flex gap-4 justify-center">
                  <button 
                    ...
                    className="
                      ...
                      text-red-100 font-serif text-xl tracking-widest uppercase
                  ...
                  <button 
                    ...
                    className="
                      ...
                      text-slate-300 font-serif text-xl tracking-widest uppercase
